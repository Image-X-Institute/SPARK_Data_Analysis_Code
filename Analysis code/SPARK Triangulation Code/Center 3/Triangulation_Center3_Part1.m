...
imThresh1 = 0.8; %For autoMV segmentation: choose values [0 1] where autosegmentation performance is poor.
...
if offset >=0
   ...
   kVData.MVGantryAngle = kVCSharpData.GantryAngle(FrameNoIndex + offset);
...
for n = 1:size(MVData.WindowsTimestamps,1)
    ...
    %The regular triangulation code will match kV/MV images based on te
    %images timestamp: However, PMC's files don't contain the correct
    %timestamps so images are matched using a combination of the timestamp
    %and gantry angle from the file names.
    gantryDiff = abs(MVData.MVGantryAngle(n) - kVData.MVGantryAngle);
    indexOfMatch = find(timeDiff + gantryDiff == min(timeDiff+gantryDiff));
...
for n = StartNbr:SlicePerTri:numMV-SlicePerTri
    ...
    disp([n meanPixVals])
    pixThreshold = min(zoomedMVFrame(:))*0.9999; %1.025; % 25% pix value of the minimum
...
for n = 1:length(KIMData.kVFilename)
    kVFilename(n,:) = sscanf(KIMData.kVFilename(n,:), '%*[^V]%*[^C]%*[^V]%*[^C]Ch1_%f_%f');
...
for n = 1:length(MVData.MVFilename)
    ...
    MVFilename(n,:) = sscanf(MVData.MVFilename{n}, 'Ch0_%f_%f_%f');
end
...
MVData.MVGantryAngle = MVFilename(:,3);
MVFilename = MVFilename(:,2);
