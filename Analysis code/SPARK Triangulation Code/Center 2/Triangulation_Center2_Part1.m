...
%----- INPUT ----%
CreateMVandkKVTimeStampsFromCSharp = 'NO'; %Create timetamps for kV and MV from Csharp code - saves time if already created.
DisplayImage = 'YES';
%----------------%

imThresh1 = 0.75; %For autoMV segmentation: choose values [0 1] where autosegmentation performance is poor.
imThresh2 = 0.7; %For MV image appearance: choose values [0 1] where higher values make the image appear darker
...
j = 1;
for n = 1:length(kVData.kVFrameNo)
    if (kVData.kVFrameNo(n)<=max(kVCSharpData.kVFrameNo))%&n~= [955:962]  %& n < 980%& n~=[279:285, 765]
           try
            FrameNoIndex(n) = find(abs(kVData.kVSourceAngle(n)+90 - kVCSharpData.GantryAngle(j:end)) < 1e-5,1) +j-1;
            j = FrameNoIndex(n)+1;
        catch
            FrameNoIndex(n) = NaN;
            %j = FrameNoIndex(n)+1;
...
%     kVData.WindowsTimestamps = kVCSharpData.Timestamps(FrameNoIndex2+offset);
%     kVData.kVFrameNo = kVCSharpData.kVFrameNo(FrameNoIndex2+offset);
%     kVData.kVFilename = kVData.kVFilename(:,2);
...
for n = 1:size(MVData.WindowsTimestamps,1)
    ...
    %indexOfMatches(n) = indexOfMatch(1);
...
%figure; plot(MVData.WindowsTimestamps, kVData.WindowsTimestamps(indexOfMatches))
%hold on; plot(MVData.WindowsTimestamps(GoodSyncIndex), kVData.WindowsTimestamps(indexOfMatches))
%xlabel('MV Windows timestamps','fontsize',15)
%ylabel('kV Windows timestamps','fontsize',15)
...
%BlankMVFolder = 'D:\KIM_Development\Data\Rot_Experiments\EXP_04042016\WindowBlankMV\CH0\Arc1\';
...
for n = StartNbr:SlicePerTri:numMV-SlicePerTri
    ...
    elseif frameAverage == 1 %Change back to frameAverage == 1
    ...
    %%%if MVData.MVGantryAngle(n) > nokVViewAngle
    %if (MVData.MVGantryAngle(n) > nokVViewAngle1) || (MVData.MVGantryAngle(n) < nokVViewAngle2)
    ...
    % Flip the iTools kV image upside down. The iTools MV image does not
    % need to be flipped.
    kVFrame = 1-kVFrame;
%     kVFrame = flipud(kVFrame);
%     kVFrame2= flipud(kVFrame2);
    ...
    estMarker = [estMarker(:,1)-yStartkV+1, estMarker(:,2)-xStartkV+1];
    %estMarker = [estMarker(:,1)-yStartMV+1, estMarker(:,2)-xStartMV+1];
    autoPosition = MVforKim(zoomedMVFrame, 3, estMarker, 150, imThresh1); %3 is number of markers; 180 is SAD in cm
    ...
    meanPixVals  = findMeanPixelValues(xStartkV, yStartkV, zoomedMVFrame, proj, 11);
    disp([n meanPixVals])
    ...
    if  sum(lgc) >= 1  %sum(lgc) >=3 % if sum(lgc) >= 1: At least one marker is blocked by MLC --> Find frame only when all three markers are present
        if strcmp(DisplayImage, 'YES')
            ...
            grid on
            %title({'MV', ['Frame/Filename/Gantry = ' num2str(MVData.MVFrameNo(n)) '/' num2str(MVData.MVFilename(n)) '/' num2str(MVData.MVGantryAngle(n)) ], ['n/meanPix1/meanPix2/meanPix3 = ' num2str(n) '/' num2str(meanPixVals(1)) '/' num2str(meanPixVals(2)) '/' num2str(meanPixVals(3)) ' (' num2str(pixThreshold) ') SKIPPED']})
            ...
            hold on
            plot(proj.xp1 - xStartkV + 1, proj.yp1 - yStartkV + 1, 'b+', proj.xp2 - xStartkV + 1, proj.yp2 - yStartkV + 1, 'b+', proj.xp3 - xStartkV + 1, proj.yp3 - yStartkV + 1, 'b+', 'linewidth', 2)
            hold off
    ...
    MVppxp = [proj.xp1 - xStartkV + 1, proj.xp2 - xStartkV + 1, proj.xp3 - xStartkV + 1];
    MVppyp = [proj.yp1 - yStartkV + 1, proj.yp2 - yStartkV + 1, proj.yp3 - yStartkV + 1];
    ...
    if 1
        ...
        %    disp('Press onto the picture to segmentate the beacons')
            ...
            close(h5)
            MVbutton
            MVxp = MVxp + xStartkV - 1;
            MVyp = MVyp + yStartkV - 1;
...
for mm=1:3
    if (autoPosition(mm,2)~=0 && autoPosition(mm,1)~=0)
        aMVxp = autoPosition(mm,2) + xStartkV - 1;
        aMVyp = autoPosition(mm,1) + yStartkV - 1;
        MVbutton = 1;
...
function [meanPixVals] = findMeanPixelValues(xStartkV, yStartkV, frame, proj, tempSideLength)
% Computes the average pixel value for an 11 by 11 template centred at (y,x)
tempSize = floor(tempSideLength/2);

proj.xp1 = proj.xp1 - xStartkV + 1;
proj.xp2 = proj.xp2 - xStartkV + 1;
proj.xp3 = proj.xp3 - xStartkV + 1;

proj.yp1 = proj.yp1 - yStartkV + 1;
proj.yp2 = proj.yp2 - yStartkV + 1;
proj.yp3 = proj.yp3 - yStartkV + 1;
...
elseif detectorType == 'MV'
    ...
    SDD = linac.MVSDD;
    linac.XCenterPixel = linac.MVXCenterPixel;
    linac.YCenterPixel = linac.MVYCenterPixel;
...
linacData.MVPixelSizeAtIso = 0.336; % in mm
...
linacData.MVXWidth = 1190;
linacData.MVYWidth = 1196;

linacData.MVXCenterPixel = 595;
linacData.MVYCenterPixel = 598;
...
if OLdata ==1
    ...
    for n = 1:noOfOLTrajFiles
        ...
        rawKIMDataCurrent = textscan(fid, '%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f', 'headerLines', 0);
        end
...
elseif OLdata ==0
    ...
    for n = 1:noOfOLTrajFiles
        fid=fopen([kVFolder '\MarkerLocationsGA_CouchShift_' num2str(n-1) '.txt']);
        rawKIMDataCurrent = textscan(fid, '%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f, %*[^\n]', 'headerLines', 1);
        eval(['rawKIMData' num2str(n) '=rawKIMDataCurrent']);
        fclose(fid);
    end
end
...
[C ia] = unique(sortedFrameNo(:,1:2),'rows','last');
sortedFrameNo = sortedFrameNo(ia,:);
