...
imThresh2 = 0.4; %For MV image appearance: choose values [0 1] where higher values make the image appear darker
...
StartNbr = 10;
SlicePerTri = 6;
extraImgsOffset = 0;%Sync between KIM log and kV images
...
frameAverage = 3; % MVFrame average used for KIM fraction
...
if (strcmp(strtrim(list(4,1:2)), 'CH') == 1)
    ...
    MVFolder = [fxPath '\MV\']; %'\CH0\'
    kVFolder = [fxPath '\KV\']; %'\CH1\'
end
...
% Angle more than which there is no kV view
nokVViewAngle1 = 20;
nokVViewAngle2 = -130;
...
%yStartMV = 261; xStartMV = 378; yEndMV = 521; xEndMV=638;
...
for n = 1:length(kVData.kVFrameNo)
    ...
    %    FrameNoIndex(n) = find(kVData.kVFrameNo(n) == kVCSharpData.kVFrameNo);
        FrameNoIndex2(n) = find(kVData.kVFilename(n) == kVCSharpData.kVFilename);
    %end
end
...
hold on
plot(FrameNoIndex2,kVData.WindowsTimestamps,'linewidth',2)
...
for n = StartNbr:SlicePerTri:numMV-SlicePerTri
        ...
        if ~isempty(MVFolder)
            ...
            if ~isempty(MVFramegrabber)
                for i=1:3%SlicePerTri
                    ...
                    MVFilenames(i) = sscanf(MVFilename, 'Frame%u')';
                end
            end
        end
    ...
    if 3<2
        if ~isempty(kVFolder)
            ...
            if ~isempty(kVFramegrabber)
                ...
                kVFilename = sscanf(kVFilename, 'Frame%u');
            end
        end
    else
        if ~isempty(kVFolder)
            ...
            if ~isempty(kVFramegrabber)
                ...
                kVFilename2 = sscanf(kVFilename2, 'Frame%u');
    ...
    disp([n meanPixVals])
    pixThreshold = min(zoomedMVFrame(:)) * 1.0001; % 25% pix value of the minimum
    lgc = (meanPixVals < pixThreshold);
    
    % Branch A: Number of markers blocked by MLC (skip image)
    if sum(lgc) >=3 % if sum(lgc) >= 1: At least one marker is blocked by MLC --> Find frame only when all three markers are present
        h3 = figure(9);
        colormap gray
...
linacData.MVPixelSizeAtIso = 0.392; % in mm

linacData.MVSDD = 1.8; % in m
linacData.kVSDD = 1.8; % in m

linacData.XWidth = 1024;
linacData.YWidth = 768;

linacData.XCenterPixel = 512;
linacData.YCenterPixel = 384;
...
for n = 1:length(KIMData.kVFilename)
    kVFilename(n,:) = sscanf(KIMData.kVFilename(n,:), '%*[^F] Frame%f');
end
...
for n = 1:length(MVData.MVFilename)
    MVFilename(n) = sscanf(MVData.MVFilename{n}, 'Frame%f');
end
...
for n = 1:length(kVCSharpData.kVFilename)
    kVFilename(n) = sscanf(kVCSharpData.kVFilename{n}, 'Frame%f');
end
