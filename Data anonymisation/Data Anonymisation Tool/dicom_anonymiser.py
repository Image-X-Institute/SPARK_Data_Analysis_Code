...
def _getResourcePath(relativePath: str) -> str:
    return os.path.join(os.environ.get("_MEIPASS2", os.path.abspath(".")), relativePath)
...
def anonymise(
    ...
    if os.path.isdir(filePath):
        for subpath in os.listdir(filePath):
            numberOfFileAnonymised += anonymise(
                filePath + "/" + subpath,
        ...
        for tagToBeAnonymised in deIdentificationConfig["tags"]:
            dicomTag = (
                int(tagToBeAnonymised["group"], base=16),
            ...
            if dicomTag in ds:
                ...
                if tagToBeAnonymised["action"] == "delete":
                    ...
                    if "overwrite_with" not in tagToBeAnonymised:
                        print(
                            "Invalid dicom anonymisation configuration: "
                        ...
                        if tagToBeAnonymised["overwrite_with"]["value_type"] == "fixed":
                            ds[dicomTag].value = tagToBeAnonymised["overwrite_with"][
                                "value"
                            ]
                        elif (
                            tagToBeAnonymised["overwrite_with"]["value_type"]
                            == "pattern"
                        ):
                ...
                if verbose:
                    print(
                        f" Anonymising {tagToBeAnonymised['name']} ",
...
def prepareArgumentParser():
    ...
    argParser.add_argument(
        "--config",
        dest="anonymisationConfig",
        help="Configuration file with dicom anonymisation "
        "rules (expected in JSON format)",
        type=str,
    )
...
def dicomAnonymizer(
    ...
    anonConfig = getDefaultDeIdentificationConfig()
    numAnon = anonymise(
        filePath,
        anonConfig,
